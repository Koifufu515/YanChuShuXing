...
- **真实数据闭环**：查询确定性 SQLite 模拟数据，返回表格、关键指标、中文业务结论和技术详情。
...
- 当前使用小规模确定性模拟数据，不代表真实银行生产数据规模、权限体系或数据质量。
...
本仓库只允许提交模拟银行数据。禁止上传真实客户信息、银行内部数据、API Key、Token、Cookie 或本地 `.env`。安全边界与漏洞反馈流程见 [SECURITY.md](SECURITY.md)。
