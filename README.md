...
当前已经完成的产品与工程基础包括 Rule First、LLM Extension、SQL Safety、只读执行、基础 Metadata、模板式业务结论和网页 Demo。真实数据已完成接入前结构审计与安全边界设计，正式 ETL、Real Schema 和200题评测仍属于第一阶段任务。项目当前处于“第一阶段：真实数据基础与评测基座建设”，不是全部赛题能力完成状态。
...
- [赛题要求与能力缺口分析](docs/赛题要求与能力缺口分析.md)：按已经实现、部分实现、仅有架构基础和尚未实现核对当前能力。
- [第二阶段技术路线图](docs/第二阶段技术路线图.md)：第一阶段完成后的能力优先级、依赖、角色和启动门槛。
...
- `conversation_id` 目前只是请求标识，尚未实现会话历史存储、上下文召回和连续追问。
- `chart_hint` 目前只是内部预留字段，尚未实现智能图表推荐；业务解释仍以三个Demo问题的模板摘要为主。
- 表与字段安全检查已经实现，但用户角色、机构范围、动态脱敏等细粒度权限尚未完成；审计目前使用 NoOp Adapter，尚未持久化。
- 尚未完整实现机构与指标下钻、CSV/XLSX导出、可保存看板，也未在官方200题上验证端到端三秒性能目标。
- 尚未完成 Docker、标准 Linux 或国产化环境交付验证；FastAPI v1 和 Ports 提供适配基础，但没有真实银行中台、身份平台或模型网关联调证据。
- 尚未实现 RAG、向量检索或正式桌面/移动端应用。
...
- [第二阶段技术路线图](docs/第二阶段技术路线图.md)：第一阶段验收后的五类候选能力，不代表已经启动。
