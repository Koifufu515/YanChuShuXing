...
- 真实赛题数据到位后，删除六个 Issue 的空泛背景章节，重点将#4、#5调整为受控数据审计、清洗映射、双环境ETL和真实数据问题指标体系，并为产品、前端和测试补充数据依赖与安全约束。
- 以官方13家机构、21项指标、486个日期、132,678条事实记录和200题为真实数据范围基线，规划 `evaluation/` 评测框架及Rule、LLM、SQL、答案、耗时和分难度指标。
- 新增竞赛数据与智算平台资料的脱敏说明和 Git 忽略规则；原始数据、标准答案及含联系方式的手册不进入公开仓库。
- 将团队《Issues 不足分析》整理为正式赛题能力缺口审计，按已实现、部分实现、仅有架构基础和尚未实现校准当前能力。
- 新增第二阶段技术路线图和五份候选 Issue 草案；明确复杂NL2SQL与官方评测优先，第一阶段完成前不创建新任务或开发高级功能。
