...
- 原地更新 GitHub Issue #1 至 #6，形成项目统筹、产品方案、前端集成、官方数据治理、问题语义和固定题库评测六项工作流。
...
- 细化Issue #5为官方题库语义映射、衍生口径与异常审核职责，新增三项可人工审核且可程序读取的业务资产模板，明确05号不负责SQL、测试执行或Rule/LLM路由。
- 将正式评测统一为系统结果标准化后直接对比官方答案，删除旧双查询对照设计，并明确Issue #6的批量评测、错误归因和版本回归职责。
- test: 新增官方题库批量评测框架（evaluation/），含受控题库提取、批量Runner、标准化判分、第一层归因、专项用例库与版本回归对比；受控资产仅存 data/private/evaluation/。
