...
> **文档类型：历史实施记录，不代表当前完整能力。**
> 适用版本：Sprint 3 规则型纵向原型。当前系统已增加 DeepSeek、Rule First Hybrid、Metadata 和产品页面；当前状态以根目录 `README.md` 为准。
...
**后续状态（2026-07-14）：** 上述“当前已知问题”和“下一阶段建议”仅反映 Sprint 3 当时状态。真实 LLM、Streamlit 和 Rule First Hybrid 已在后续 Sprint 完成；旧 `models/query.py` 与 `services/pipeline.py` 经全仓库引用审计确认后已移除。当前状态以 README 和当前接口契约为准。
