...
## 当前阶段
第一阶段：真实数据基础与评测基座建设。当前执行现有#1至#6；第二阶段候选能力仅完成路线图和草案，不进入开发。
...
- [x] 推送分支并创建 Pull Request（后续由真实数据阶段替代 PR 统一承接）
- [x] 同步桌面副本并复核远端状态
- **状态：** complete
...
- [x] 创建文档提交、推送独立分支并创建替代 Pull Request（后续由真实数据阶段替代 PR 统一承接）
- [x] 同步桌面项目副本
- **状态：** complete

### 阶段 18：真实数据阶段 Issue 定向更新
- [x] 审计赛题数据包和智算平台手册的结构、访问方式和公开风险
- [x] 确认数据含受控评测答案、手册含个人联系方式，原件不得进入 Public GitHub
- [x] 删除#1至#6所有“为什么现在要做”章节
- [x] 保持 Issue 编号、标题、负责人和总体角色方向不变
- [x] 将#4定稿为五部分真实数据治理、规范化建模和数据库重构任务
- [x] 将#5定稿为官方200题分类、指标体系、Gold SQL和评测基准任务
- [x] 为#2、#3、#6补充真实数据依赖、双环境验收和禁止写死/泄露要求
- [x] 将#6扩展为200题自动评测，规划Rule/LLM/SQL/答案/耗时和分难度指标
- [x] 更新团队规范、任务清单、README、安全政策和当前数据文档
- [x] 将原始附件复制到Git忽略的本地受控目录并核对哈希
- [x] 验证Issue元数据、忽略规则、文档链接、敏感信息和87项回归测试
- [x] 创建指定提交、推送独立分支并创建替代PR
- [x] 同步桌面项目副本
- **状态：** complete

### 阶段 19：赛题能力缺口审计与第二阶段路线图
- [x] 从当前代码、测试、赛题材料和团队阶段性审计结论核对能力事实
- [x] 新增赛题要求与能力缺口分析，按已实现、部分实现、尚未实现、仅有架构基础分类
- [x] 明确第一阶段六个Issue的完成边界和第二阶段启动门槛
- [x] 新增按依赖排序的第二阶段技术路线图和六人角色映射
- [x] 生成五份第二阶段Issue草案，不在GitHub正式创建新Issue
- [x] 更新README、团队规范、首轮任务清单、CHANGELOG、progress和task_plan
- [x] 验证文档链接、敏感信息、变更边界和87项自动化测试
- [x] 创建指定提交、推送分支并创建以PR #9为前置的Pull Request
- [x] 同步桌面项目副本
- **状态：** complete

## 关键问题
1. 如何避免项目沦为“套壳大模型生成 SQL”？
2. 如何在公开Demo与受控真实数据双环境下兼顾可复现、业务价值、隐私和赛题资产保护？
...
| Demo与Real数据环境严格分离 | 公开开发可复现，同时保护受控赛题数据、标准答案和真实验证结果 |
...
| `/api/v1/ask` 返回 HTTP 500 | 1 | 已定位为早期占位 Provider 主动抛错；该历史问题已由后续可运行生成器与统一API处理替代 |

## 下一阶段
- 继续完成第一阶段：项目负责人分配#2至#6并确认真实数据私有共享位置，优先启动#4数据治理、#5问题指标与Gold SQL、#6评测基线；未达到第一阶段完成条件前不创建第二阶段正式Issue。
