...
### 阶段 17：首轮 GitHub Issues 详细化
- [x] 读取现有六个 Issue、角色文档、任务清单和实际项目目录
- [x] 保留 Issue 编号、标题、负责人和总体方向
- [x] 为六个角色分别补充背景、边界、执行顺序、参考文件、依赖、交付和完成定义
- [x] 原地更新 #1 至 #6，不新建、关闭或删除 Issue
- [x] 将任务清单精简为索引、依赖和共同关卡，避免重复长正文
- [x] 在团队规范和 README 增加详细 Issue 入口
- [x] 核对 Issue 结构、标题、负责人和仓库文档一致性
- [x] 完成链接、差异和87项自动化回归验证
- [ ] 创建文档提交、推送独立分支并创建替代 Pull Request
- [ ] 同步桌面项目副本
- **状态：** in_progress
