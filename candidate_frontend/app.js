(function () {
  ...
  function makeMultiMetricRankingCards(view) {
    if (view.structured?.answerType !== "ranking") return null;
    const metricIndex = view.columns.findIndex(column => ["指标", "指标名称"].includes(column));
    const valueIndex = view.columns.findIndex(column => ["指标值", "数值"].includes(column));
    const unitIndex = view.columns.findIndex(column => column === "单位");
    const rankIndex = view.columns.findIndex(column => ["全省排名", "排名"].includes(column));
    const institutionIndex = view.columns.findIndex(column => ["机构", "机构名称"].includes(column));
    if (metricIndex < 0 || valueIndex < 0 || rankIndex < 0 || view.rows.length < 2) return null;

    const metrics = new Set(view.rows.map(row => String(row[metricIndex] ?? "")));
    const institutions = institutionIndex < 0
      ? new Set([""])
      : new Set(view.rows.map(row => String(row[institutionIndex] ?? "")));
    if (metrics.size < 2 || institutions.size !== 1) return null;

    const grid = node("div", "ranking-kpi-grid");
    view.rows.forEach(row => {
      const item = node("section", "ranking-kpi");
      const metric = node("span", "ranking-kpi-label", valueOrMissing(row[metricIndex]));
      const value = valueOrMissing(row[valueIndex]);
      const unit = unitIndex >= 0 ? String(row[unitIndex] ?? "") : "";
      const rank = node("strong", "ranking-kpi-value", `${value}${unit}`);
      const place = node("small", "ranking-kpi-rank", valueOrMissing(row[rankIndex]));
      item.append(metric, rank, place);
      grid.append(item);
    });
    return grid;
  }
  ...
  function renderAnswer(turn, index) {
    ...
    if(waiting){ body.append(renderConfirmation(turn,index,payload)); }
      ...
      const rankingCards = makeMultiMetricRankingCards(view);
      if (rankingCards) body.append(rankingCards);
